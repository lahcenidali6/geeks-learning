...
export default function App() {
  ...
  const bgClass = backgroundColors[currentIndex % backgroundColors.length];

  return (
    <div className={`d-flex align-items-center justify-content-center vh-100 p-4 ${bgClass}`}>
      <motion.div
      ...
      >
        <div className="card shadow rounded-4 p-4 bg-white bg-opacity-75" style={{ minHeight: '300px', backdropFilter: 'blur(10px)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div className="card-body text-center flex-grow-1 d-flex flex-column justify-content-center">
            <AnimatePresence mode="wait">
              {showQuote && (
                ...
                >
                  <p className="fs-4 fst-italic fw-medium mb-4 text-dark" style={{ minHeight: '96px' }}>"{quote}"</p>
                  <p className="fs-5 fw-semibold text-secondary mb-4">{author}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
...
export function Card({ children, className = '', style = {} }) {
  return (
    <div
      className={`card shadow rounded-4 bg-white bg-opacity-75 ${className}`}
...
export function Button({ children, onClick, className = '', ...props }) {
  return (
    <button
      type="button"
      className={`btn btn-primary rounded-pill px-4 py-2 ${className}`}
      onClick={onClick}
      {...props}
    >
      {children}
